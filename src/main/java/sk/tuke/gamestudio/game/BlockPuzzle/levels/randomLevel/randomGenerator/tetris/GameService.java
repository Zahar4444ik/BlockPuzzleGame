...
public class GameService {
    public sk.tuke.gamestudio.DTO.game.GameStateDTO updateGameState(GameUpdateRequestDTO request) {
        ...
        Map<Position, Block> blockPositions = convertToPositionBlockMap(currentState.getBlockPositions());
        Board board = new Board(grid.length, grid[0].length);
        board.setGrid(grid);
        board.setBlockMap(placedBlocks);
        board.setBlockPositions(blockPositions);
    ...
    public sk.tuke.gamestudio.DTO.game.GameStateDTO convertToGameStateDTO(Board board, List<Block> availableBlocks) {
        return new GameStateDTO(
                ...
                convertToPlacedBlocksDTO(board.getBlockPositions()),
                board.isFull()
        );
    }
